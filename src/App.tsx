      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-white/90 backdrop-blur-md py-4'}`}>
        <nav className="container mx-auto px-4 flex items-center justify-between">
          <a href="#" className="flex items-center gap-3 group">
            <div className="w-10 h-10 bg-maipu-green rounded-lg flex items-center justify-center text-white font-display font-bold text-xl group-hover:rotate-3 transition-transform">M</div>
            <span className="font-display text-2xl font-bold text-maipu-green tracking-tight">CLUB MAIPÚ</span>
          </a>

          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} className="text-sm font-semibold text-maipu-green uppercase tracking-wider hover:text-maipu-light-green transition-colors">
                {link.name}
              </a>
            ))}
            <button onClick={() => setIsSocioModalOpen(true)} className="bg-maipu-green text-white px-6 py-2 rounded-full font-bold hover:bg-maipu-accent transition-all shadow-lg text-sm">
              ¡HACETE SOCIO!
            </button>
          </div>

          <button className="md:hidden text-maipu-green p-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {isMenuOpen && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="md:hidden bg-white border-t border-slate-100 overflow-hidden">
              <div className="container mx-auto px-4 py-6 flex flex-col space-y-4">
                {navLinks.map((link) => (
                  <a key={link.name} href={link.href} onClick={() => setIsMenuOpen(false)} className="text-lg font-bold text-maipu-green uppercase">
                    {link.name}
                  </a>
                ))}
                <button onClick={() => { setIsSocioModalOpen(true); setIsMenuOpen(false); }} className="bg-maipu-green text-white w-full py-3 rounded-md font-bold">
                  ¡HACETE SOCIO!
                </button>
              </div>
            </motion.div>
      ...
      <main className="pt-20">
        ...
        <section className="relative h-[85vh] min-h-[600px] flex items-center justify-center text-white overflow-hidden">
          <div className="absolute inset-0 z-0">
            <img alt="Club Maipú Atmosphere" className="w-full h-full object-cover scale-105" src={generalInfo.imagenFondo} referrerPolicy="no-referrer" />
          ...
          <div className="relative z-10 container mx-auto px-4 text-center">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <h1 className="font-display text-6xl md:text-8xl font-bold mb-6 drop-shadow-2xl leading-tight">
                {generalInfo.titulo}<br/>
                <span className="text-maipu-light-green">{generalInfo.subtitulo}</span>
              </h1>
              <p className="text-xl md:text-2xl mb-12 max-w-2xl mx-auto font-light italic opacity-90">
                {generalInfo.descripcion}
              </p>
              <div className="flex flex-col sm:flex-row gap-6 justify-center">
                <a href="#disciplinas" className="bg-white text-maipu-green px-10 py-4 rounded-md font-bold text-lg hover:bg-slate-100 transition-all shadow-xl uppercase tracking-widest">
                  Nuestras Actividades
                </a>
                <button onClick={() => setIsSocioModalOpen(true)} className="border-2 border-white text-white px-10 py-4 rounded-md font-bold text-lg hover:bg-white hover:text-maipu-green transition-all uppercase tracking-widest backdrop-blur-sm">
                  Sumate al Club
                </button>
              </div>
            </motion.div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {disciplinas.map((discipline: any, index: number) => (
                <motion.div key={discipline.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="group relative overflow-hidden rounded-2xl shadow-xl hover-lift aspect-[4/5]">
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {noticias.map((item: any, index: number) => (
                <motion.article key={item.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="bg-white rounded-2xl shadow-sm overflow-hidden border border-slate-200 hover:shadow-xl transition-all group">
                  <div className="overflow-hidden h-56">
                    <img alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" src={item.image} referrerPolicy="no-referrer" />
                  </div>
                  <div className="p-8">
              ...
              <div>
                ...
                <div className="space-y-10">
                  <div className="flex items-start gap-6 group">
                    <div className="bg-slate-50 p-4 rounded-2xl text-maipu-green group-hover:bg-maipu-green group-hover:text-white transition-colors"><MapPin size={32} /></div>
                    <div>
                      <h4 className="font-display text-xl font-bold text-maipu-green mb-1">Dirección</h4>
                      <p className="text-slate-600">Av. General Paz 1234, Maipú, Mendoza</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-6 group">
                    <div className="bg-slate-50 p-4 rounded-2xl text-maipu-green group-hover:bg-maipu-green group-hover:text-white transition-colors"><Phone size={32} /></div>
                    <div>
                  ...
                  <div className="flex items-start gap-6 group">
                    <div className="bg-slate-50 p-4 rounded-2xl text-maipu-green group-hover:bg-maipu-green group-hover:text-white transition-colors"><Clock size={32} /></div>
                    <div>
